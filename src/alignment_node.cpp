...
public:
    AlignmentSolver() : Node("alignment_solver") {
        init_comms();
        RCLCPP_INFO(get_logger(), "== SOLVER READY ==");
    }
...
private:
    ...
    void init_comms() {
        // 1. LIDAR SLAM Input (PoseWithCovarianceStamped)
    ...
    // Logic 1: Save Location พร้อมการเช็ค Data & Time Sync
    void on_save_location(const std_srvs::srv::Trigger::Request::SharedPtr, std_srvs::srv::Trigger::Response::SharedPtr res) {
        ...
        // 1.2 เช็ค Time Difference (ห้ามเกิน 1 วินาที)
        ...
        res->success = true;
        res->message = "Point #" + std::to_string(pts_lidar_.size()) + " saved successfully. (Diff: " + std::to_string(time_diff) + "s)";
        RCLCPP_INFO(get_logger(), "%s", res->message.c_str());
    }

    // --- Logic 2: Calculate พร้อม Reset และ Dynamic Frames ---
    void on_calculate(const pttep_alignment::srv::CalculateTransformation::Request::SharedPtr req,
        ...
        if (req->reset) {
            ...
            pts_gps_.clear();
            res->success = true;
            res->message = "Data reset successfully.";
        ...
        // คำนวณ SVD
        auto [R, t] = compute_svd();

        // Response (Dynamic Frame Mapping)
        res->transform.header.stamp = now();
        res->transform.header.frame_id = last_gps_msg_->header.frame_id; // Frame จาก GPS (***)
        res->transform.child_frame_id = last_lidar_msg_->header.frame_id; // Frame จาก SLAM (**)
        ...
        res->success = true;
        res->message = "Alignment successful.";
    ...
    // --- Math Engine: SVD ---
    std::pair<Eigen::Matrix2d, Eigen::Vector2d> compute_svd() {
        int n = pts_lidar_.size();
        Eigen::Vector2d c_l(0,0), c_g(0,0);
        for(int i=0; i<n; i++) { c_l += pts_lidar_[i]; c_g += pts_gps_[i]; }
        c_l /= n; c_g /= n;

        Eigen::Matrix2d H = Eigen::Matrix2d::Zero();
        for(int i=0; i<n; i++) {
            H += (pts_lidar_[i] - c_l) * (pts_gps_[i] - c_g).transpose();
        }
